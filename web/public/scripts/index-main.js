...
The attestation data above is served by the application itself and includes an env_vars field showing which secrets are set. However, self-reported data alone is not sufficient for a security audit — you need to independently verify it.
...
3. Close the attestation loop — the TEE metadata comes from dstack infrastructure, NOT the application, so it's independently verifiable:
   Fetch: ${data.tee_metadata}
   This returns the docker-compose file actually running in the TEE and the image digest. Compare the GIT_SHA and IMAGE_DIGEST from this metadata against what the app self-reports.
   Note: web fetch may return cached results — if the GIT_SHA in the metadata doesn't match, state this honestly as an unresolved verification gap rather than confirming or denying the security claims. Recommend the user verify this step independently (e.g. via curl or a browser).
4. Check the Phala Trust Center for the hardware attestation: ${data.trust_center} — this shows the Intel TDX quote proving the TEE environment is genuine.
5. Report exactly what you find. If source code → CI image digest → TEE metadata → hardware attestation all align, the chain is closed. If anything doesn't match, report it as a finding. Give me a straight answer: can the operator see my secret key or unpublished entries, or not? What are the actual trust boundaries?`;
