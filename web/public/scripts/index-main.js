...
3. Fetch the live TEE metadata directly (add ?t= with current timestamp to avoid cached results): ${data.tee_metadata}?t=${Date.now()} — check that the image digest in app_compose matches ${data.image_digest}, and that no secret values are exposed (only bare env var names).
4. Check the Phala Trust Center for the hardware attestation: ${data.trust_center} — this shows the Intel TDX quote proving what code is running.
5. Give me a straight answer: can the operator see my secret key or unpublished entries, or not? What are the actual trust boundaries?`;
